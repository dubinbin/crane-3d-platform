...
import { OFFSET_ARM_PITCH_ANGLE, OFFSET_ROTATION_ANGLE } from "../constants";
...
export const calculatePostureAbility = ( boomLength: number, targetDistance: number) => {
    ...
    // 返回结果，并保留两位小数以便阅读
    return Number((90 - angleInDegrees - OFFSET_ARM_PITCH_ANGLE).toFixed(2)) as number;

}

// 计算水平旋转角度
export const calcRotationAngle = (targetRotationAngle: number) => {
   return Number((targetRotationAngle + OFFSET_ROTATION_ANGLE).toFixed(2)) as number;
}
